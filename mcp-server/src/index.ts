    ...
    tools: [
      {
        name: "list_files",
        description:
          "List files and folders inside the user's workspace.",
        inputSchema: {
          type: "object",
          properties: {
            directory: {
              type: "string",
              description:
                "Directory relative to the workspace. Defaults to the workspace root.",
            },
          },
        },
      },
...
async function main() {
  ...
  app.listen(PORT, () => {
    console.error(
      `Workspace MCP server running on http://localhost:${PORT}`
    );
